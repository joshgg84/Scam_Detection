...
bot.command('addtip', async (ctx) => {
    ...
    const parts = ctx.message.text.split(' ');
    if (parts.length < 2) {
        ctx.reply('📝 *Usage:* `/addtip Your new security tip here`', { parse_mode: 'Markdown' });
        return;
    }
